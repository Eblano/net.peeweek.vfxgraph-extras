...
public class VolumeMixerSettings : ScriptableObject
{
    [SettingsProvider]
    ...
    const string filePath = "ProjectSettings/VolumeMixerSettings.asset";
    ...
    //singleton pattern
    static VolumeMixerSettings s_Instance;
    static VolumeMixerSettings instance => s_Instance ?? CreateOrLoad();
    VolumeMixerSettings()
    {
        s_Instance = this;
    }

    static VolumeMixerSettings CreateOrLoad()
    {
        //try load
        ...
        if (s_Instance == null)
        {
            VolumeMixerSettings created = CreateInstance<VolumeMixerSettings>();
